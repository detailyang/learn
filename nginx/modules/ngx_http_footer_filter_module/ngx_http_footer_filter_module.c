...
ngx_http_footer_header_filter(ngx_http_request_t *r)
{
    ...
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "headerrrrrrrrrr");
...
// 局部变量定义，类型名短的在前。变量名对齐，“*”不算在变量名中。
{
    ...
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "inject ctx");
    ...
    cl->next = nl;
    cl->buf->last_buf = 0;
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "buffffer:%V", &ctx->footer);
...
{
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, "filter init");
